...
public final class KiekerServiceCallRecordFilter extends AbstractFilterPlugin implements ServiceCallDataSet {
    ...
    @Override
    public List<String> getExternalServiceCallsIds(){
    	List<ServiceCall> external = this.allServiceCalls;
    	external.removeIf(s -> (!s.getCallerId().isPresent()));
    	if(external.isEmpty()) return Collections.EMPTY_LIST;
    	List<String> ids=external.stream().map(s->s.getServiceId()).distinct().collect(Collectors.toList());
    	return ids;
    }
