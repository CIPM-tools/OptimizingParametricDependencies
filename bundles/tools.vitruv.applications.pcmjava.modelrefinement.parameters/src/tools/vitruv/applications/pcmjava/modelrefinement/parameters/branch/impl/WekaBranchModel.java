...
public class WekaBranchModel implements BranchModel {
	...
	public WekaBranchModel(final WekaDataSet<String> dataset, final String branchNotExecutedId) throws Exception {
	...
	public String getBranchStochasticExpression(final String transitionId) {
		int index = this.dataset.getClassAttribute().indexOfValue(transitionId);
		if(index == -1) return "";
		return this.classifier.getBranchStochasticExpression(index, this.attributeExpressions);
	}
