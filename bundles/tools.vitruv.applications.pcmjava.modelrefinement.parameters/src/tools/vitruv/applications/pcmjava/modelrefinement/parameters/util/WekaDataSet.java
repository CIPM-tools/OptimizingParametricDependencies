...
import weka.core.Attribute; //added manually
...
public class WekaDataSet<T> {
	...
	private static final Logger LOGGER = Logger.getLogger(WekaDataSet.class);
	...
	public WekaDataSet(final List<ParametersWithClass<T>> values, final WekaDataSetMode mode) {
		...
		// build "columns header" of the dataset
	...
	private static <T> WekaClassAttribute getClassAttribute(List<ParametersWithClass<T>> values) {
		Object basedOnClassValue = values.get(0).getClassValue(); // Object of type T
		if (basedOnClassValue instanceof String || basedOnClassValue instanceof Boolean) {
	...
	public Map<String, List<Object>> getAttrForValues() {
		Map<String, List<Object>> attrToValues = new HashMap<String, List<Object>>();
		for (Attribute attr : this.attributes) {
			List<Object> values = new ArrayList<Object>();
			for (Instance ins : this.getDataSet()) {
				if (attr.isNumeric()) {
					values.add(ins.value(attr));
				} else {
					values.add(ins.stringValue(attr));
				}
			}
			attrToValues.put(attr.name(), values);
		}
		return attrToValues;
	}
	...
	public List<String> getInstances() {
		...
		for (Instance ins : this.getDataSet()) {
			instancesList.add(ins.toString());
		}
	...
	private Instance buildInstance(final ParametersWithClass<T> value) {
	...
	public Map<String, Integer> getAttributesWithIndex() {
		Map<String, Integer> result = new HashMap<String, Integer>();
		this.attributes.forEach(a -> result.put(a.name(), a.index()));
		return result;
	}
	...
	public Map<String, List<WekaServiceParameter>> getParametersToAttributes() {
		return parametersToAttributes;
	}
	...
	private void addNumericParameter(final String name, final List<ParametersWithClass<T>> values) {
		...
		int index = this.parameters.size(); // where to add the new parameter
		newParameters.add(new NumericWekaServiceParameter(name, index));
		index++;
//[edit] With the implemented optimization we no longer need to add these parameters -S.V. 28.02.2020
...
//        }
	...
	private void addParameter(final String name, final List<ParametersWithClass<T>> values) {
		...
		} else if ((value instanceof String || value instanceof Character) && this.mode != WekaDataSetMode.IntegerOnly 
				&& this.mode != WekaDataSetMode.NumericOnly) {
			this.addStringParameter(name, values);
		} else {
			LOGGER.warn("Handling parameter of type " + value.getClass().getSimpleName() + " in mode " + this.mode + " is not implemented. " + name);
		}
	}
	...
	private static class StringWekaClassAttribute<T> extends WekaClassAttribute {
		...
		private static <T> ArrayList<String> createPossibleValues(final List<ParametersWithClass<T>> values) {
			...
			for (ParametersWithClass<T> parameters : values) {
				String value = String.valueOf(parameters.getClassValue());
		...
		@Override
		public double getValue(final Object value) {
			return this.getWekaAttribute().indexOfValue(String.valueOf(value));
		}
	}
	...
	private static class StringWekaServiceParameter<T> extends WekaServiceParameter {
		...
		private static <T> ArrayList<String> createPossibleValues(final String parameterName,
			...
			Set<String> possibleValues = new HashSet<String>();
			for (ParametersWithClass<T> parameters : values) {
				String value = String.valueOf(parameters.getParameters().getParameters().get(parameterName));
				possibleValues.add(value);
			}
			return new ArrayList<String>(possibleValues);
		}
		...
		@Override
		public void setValue(final Object value, final double[] result) {
			result[this.getIndex()] = this.getWekaAttribute().indexOfValue(String.valueOf(value));
		}
	}
	...
	private static abstract class WekaServiceParameter {
		...
		public String getStochasticExpression() {
			return this.getParameterName(); // a.VALUE
		}
