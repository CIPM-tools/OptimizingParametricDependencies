...
public class WekaDataSet<T> {
	...
	public List<String> getInstances(boolean numericOnly) {
		List<String> instancesList = new ArrayList<String>();
		for(Instance ins : this.getDataSet()) {
			//if all attributes in the dataset are numeric
			if(numericOnly && this.attributes.stream().filter(a->a.isNumeric()).count() == this.attributes.size()) {
    ...
    private void addNumericParameter(final String name, final List<ParametersWithClass<T>> values) {
        ...
        int index = this.parameters.size(); //from here we can insert the new column with parameters
...
//        }
    ...
    private void addParameter(final String name, final List<ParametersWithClass<T>> values) {
        ...
        } else if (value instanceof String && this.mode != WekaDataSetMode.IntegerOnly //edit: it was "instanceof Double"
                && this.mode != WekaDataSetMode.NumericOnly) {
